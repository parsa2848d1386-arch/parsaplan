                        ...
                        {pendingActions?.tasks && (
                            <div className="bg-white dark:bg-gray-800 border border-indigo-100 dark:border-indigo-900/50 rounded-2xl p-4 shadow-xl shadow-indigo-100/50 dark:shadow-none animate-in fade-in slide-in-from-bottom-5 mb-4">
                                <div className="flex items-center justify-between mb-4 border-b border-gray-100 dark:border-gray-700 pb-3">
                                    <h4 className="font-bold text-sm text-indigo-600 dark:text-indigo-400">پیش‌نمایش ({pendingActions.tasks.length} تسک)</h4>
                                </div>
                                <div className="space-y-3 max-h-[40vh] overflow-y-auto pr-2 custom-scrollbar">
                                    {pendingActions.tasks.map((task, idx) => (
                                        <div key={idx} className="bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-700 rounded-xl p-3">
                                            <div className="grid grid-cols-1 gap-2">
                                                <input type="text" value={task.title} onChange={(e) => handleTaskEdit(idx, 'title', e.target.value)} className="bg-transparent font-bold text-gray-800 dark:text-white border-none p-0 focus:ring-0 text-sm" />
                                ...
                                <div className="flex gap-3 mt-4 pt-3 border-t border-gray-100 dark:border-gray-700">
                                    <button onClick={confirmTasks} className="flex-1 bg-green-600 text-white h-10 rounded-xl text-sm font-bold hover:bg-green-700 transition flex items-center justify-center gap-2"><Check size={16} /> ثبت</button>
